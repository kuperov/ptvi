...
from scipy.sparse import (
    spdiags, block_diag, kron as spkron, identity as speye, csc_matrix,
    bmat as spbmat)
...
def svm_sampler(y, nloop=55_000, burnin=5_000, is_dym_prob=False,
                is_alp_const=False, rs = None):
    ...
    assert not is_alp_const or is_dym_prob, \
        'For UC-SVM-const dynamic probabilities cannot be computed'

    # prior
    phi0 = .97; Vphi = .1**2
    mu0 = 0; Vmu = 10
    Vgam = 10*np.eye(2); invVgam = np.linalg.inv(Vgam)
    beta0 = 0; Vbeta = 10
    nuOmega = 10
    if is_alp_const:
    ...
    H = spdiags([np.ones([2*T]), -np.ones([2*T])], [0, -2], m=2*T, n=2*T,
                format='csr')
    Hphi = spdiags([np.ones([T]), -phi*np.ones([T])], [0, -1], m=T, n=T,
                   format='csr')
    ...
    for loop in range(1, nloop+1):
        # sample gam
        invOmegagam = spbmat([[invVgam, invOmegagamB1],
        ...
        # sample h
        HinvSH = Hphi.T @ spdiags(
            np.concatenate([[1-phi**2], np.ones(T-1)])/sig2, 0, T, T) @ Hphi
        deltah = spsolve(
            Hphi, np.concatenate([[mu], mu*(1-phi)*np.ones(T-1) + y[:-1]*beta]))
        ...
        hstar = ht
        uh = hstar - deltah
        logc = (-0.5 * uh.T @ HinvSH @ uh - 0.5 * np.sum(hstar)
            - 0.5 * np.exp(-hstar).T @ (y-tau-alp*np.exp(hstar))**2 + np.log(3))
        flag = 0
        while not flag:
            hc = ht + spsolve(cholHh.T, rs.normal(size=T))
            vhc = hc - ht
            uhc = hc - deltah
            alpARc = (-0.5 * uhc.T@HinvSH@uhc - 0.5 * np.sum(hc)
                - 0.5 * np.exp(-hc).T@(y - tau - alp * np.exp(hc)) ** 2
                + 0.5 * vhc.T@Kh@vhc - logc)
            if alpARc > np.log(rs.uniform()):
                flag = 1
        # MH-step
        vh = h-ht
        uh = h-deltah
        alpAR = (-0.5 * uh.T@HinvSH@uh - 0.5 * np.sum(h)
        ...
        Omega = stats.invwishart.rvs(scale=SOmega + err.T@err, df=nuOmega + T-1,
                                     random_state=rs)

        # sample sig2
        errh = np.concatenate([[(h[0]-mu)*np.sqrt(1-phi**2)],
        ...
        def flam(x):
            return (-(1-x[1])**2/(2*sig2)*(h[0]-x[0])**2 - 1/(2*sig2) *
                np.sum((h[1:] - x[1]*h[:-1] - x[0]*(1-x[1]))**2)
                - 1/(2*Vphi)*(x[1]-phi0)**2 - 1/(2*Vmu)*(x[0]-mu0)**2)
...
def proplam(h, sig2, rs):
    ...
    nu = 5
    δ = delt + solve(C.T, rs.normal(size=2)) / \
          np.sqrt(rs.gamma(size=1, shape=nu/2, scale=2/nu))
    lam = np.array([δ[0], np.tanh(δ[1])])
    c = special.gammaln((nu+2)/2) - special.gammaln(nu/2) - np.log(nu) - \
        np.log(np.pi) + .5 * np.log(np.linalg.det(B))

    def g(x):
        err = np.array([x[0], np.arctanh(x[1])]).T - delt
        return (c + 2 * np.log(np.cosh(np.arctanh(x[1])))   # Jacobian of transformation
...
class SVMDraws(dict):
    ...
    def __getattr__(self, item):
        if isinstance(item, str) and item.endswith('hat'):
            ...
            return np.percentile(a=self.get(item[:-2], None), q=[5., 95.],
                                 axis=0)
        else:
