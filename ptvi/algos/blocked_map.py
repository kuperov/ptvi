...
def map_global(model: Model, y: torch.Tensor, z, θ0=None, max_iters=20, ε=1e-4,
               quiet=False, **kwargs):
    """Compute the maximum a postiori (MAP) by maximizing the log joint
    function with respect to the global parameters, while holding the local
    variables z fixed.
    """
    print('map_global')
    θ = torch.tensor(θ0, requires_grad=True)
    optimizer = torch.optim.LBFGS([θ], **kwargs)
    last_loss, t = None, -time()
    for i in range(max_iters):
        def closure():
            optimizer.zero_grad()
            loss = -model.ln_joint(y, torch.cat([z, θ]))
            loss.backward()
            return loss
        ...
        if last_loss and last_loss < loss + ε:
            ...
            break
        print(f'{i:8d}. log joint = {-float(loss):.4f}')
        last_loss = loss
    else:
        print('WARNING: maximum iterations reached.')
    print(f'{i:8d}. log joint = {-float(loss):.4f}')
    t += time()
    print(f'Completed {i+1:d} iterations in {t:.2f}s @ {(i+1)/t:.2f} i/s.')
    print(_DIVIDER)
    return θ.detach()


def map_local(model: Model, y: torch.Tensor, θ, z0=None, max_iters: int=20,
               ε=1e-4, **kwargs):
    """Compute the maximum a postiori (MAP) by maximizing the log joint
    function with respect to the local parameters, while holding the global
    variables z fixed.
    """
    print('map_local')
    z = torch.tensor(z0, requires_grad=True)
    optimizer = torch.optim.LBFGS([z], **kwargs)
    last_loss, t = None, -time()
    for i in range(max_iters):
        def closure():
            optimizer.zero_grad()
...
def blocked_map(model: Model, y: torch.Tensor, ζ0=None, max_iters=20, ε=1e-4, **kwargs):
    if ζ0 is None:
        ζ0 = torch.zeros(model.d)
    z, θ = ζ0[:-4], ζ0[-4:]
    for _ in range(3):
        θ = map_global(model, y, z, θ0=θ, max_iters=max_iters, ε=1e-4, **kwargs)
        z = map_local(model, y, θ, z0=z, max_iters=max_iters, ε=1e-4, **kwargs)
