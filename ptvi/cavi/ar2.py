...
class AR2(Model):
    ...
    def ln_joint(self, y, ζ):
        ...
        llhood = Normal(μ + y[1:-1] * ρ1 + y[:-2] * ρ2, 1/torch.sqrt(τ)).log_prob(y[2:]).sum()
        lprior = (
    ...
    def simulate(self, μ, ρ1, ρ2, τ):
        ...
        y[0] = Normal(
            μ / (1 - ρ1 - ρ2),
            1/math.sqrt(τ) * math.sqrt((1 - ρ2)/((1 + ρ2)*((1 - ρ2)**2 - ρ1**2)))).sample()
        y[1] = μ + ρ1 * y[0] + Normal(
            μ / (1 - ρ2), 1 / math.sqrt(τ * (1 - ρ2**2))).sample()
        for i in range(2, self.input_length):
            y[i] = μ + ρ1*y[i-1] + ρ2*y[i-2] + Normal(0, 1/math.sqrt(τ)).sample()
        return y
    ...
    def forecast(self, ζ: torch.Tensor, y: torch.Tensor, fc_steps: int) -> \
        ...
        for i in range(2, fc_steps):
            fc[i] = μ + ρ1 * fc[i-1] + ρ2 * fc[i-2] + Normal(0, 1/math.sqrt(τ)).sample()
        return fc


def arp_sgvb_forecast(y, p, steps, fit, draws=1_000):
